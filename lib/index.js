...
export default class MarketoSource extends Activity {
    ...
    get(config) {
        return new this.MarketoSource(config.clientId,
                                      config.clientSecret,
                                      config.endpoint,
                                      config.identity);
    }
    ...
    async lookupLeads(fieldKey, fieldValue, nextPageToken = null) {
        ...
        if (response.data.nextPageToken) {
            // there is another page of data, so append it to this data
            return response.data.result + this.lookupLeads(fieldKey, fieldValue, response.data.nextPageToken);
        }
